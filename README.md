...
**問題**:
- HTMLテンプレート内の `</script>` タグがブラウザによって実際のスクリプト終了タグと認識される
- その結果、残りのJavaScriptコードがHTMLとして画面に表示される
- ページが正常にレンダリングされない
...
const html = `
    <script>
        // JavaScriptコード
    </script>  ← ここでブラウザがスクリプトを終了と判断
`;
// この後のJavaScriptがHTMLとして表示される
```
