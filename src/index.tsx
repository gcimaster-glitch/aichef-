  ...
  if (pathname === "/api/plans/generate" && req.method === "POST") {
    ...
    try {
      const allMainRecipes = await env.DB.prepare(
        `SELECT * FROM recipes WHERE role='main' AND popularity >= 5 AND is_active = 1 ${combinedFilter} ORDER BY RANDOM()`
      ).all();
      
      const allSideRecipes = await env.DB.prepare(
        `SELECT * FROM recipes WHERE role='side' AND popularity >= 5 AND is_active = 1 ${combinedFilter} ORDER BY RANDOM()`
      ).all();
      
      const allSoupRecipes = await env.DB.prepare(
        `SELECT * FROM recipes WHERE role='soup' AND popularity >= 5 AND is_active = 1 ${combinedFilter} ORDER BY RANDOM()`
      ).all();
...
app.get('/api/recipes/:recipe_id', async (c) => {
  ...
  try {
    ...
    let substitutes = [];
    try {
      if (recipe.substitutes_json) {
        substitutes = JSON.parse(recipe.substitutes_json as string);
      }
    } catch (e) {
    ...
    try {
      if (recipe.tags_json) {
        tags = JSON.parse(recipe.tags_json as string);
      }
    } catch (e) {
...
app.get('/api/recipes/stats', async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      averages: {
        time_minutes: Math.round((stats?.avg_time as number) || 0),
        child_friendly_score: Math.round((stats?.avg_child_friendly as number) || 0)
      }
    });
